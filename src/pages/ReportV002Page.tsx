...
import { Box, Container, Divider, List, ListItem, ListItemText, Stack, Table, TableBody, TableCell, TableHead, TableRow, Typography } from '@mui/material';
...
export default function ReportV002Page() {
  return (
  ...
  return (
    ...
    >
      <Stack gap={2} sx={{ flex: 1, minHeight: 0 }}>
        <PageHeader
          pageTitle="Compliance reports v002"
          breadcrumbs={
            <OverflowBreadcrumbs
              leadingElement={<span>Connected Compliance</span>}
              items={[{ id: 'report-v002', label: 'Compliance reports v002', url: '/report-v002' }]}
              hideLastItem
...
function CasesByCategoryChart() {
  const data = [
    { name: 'Harassment', value: 27 },
    { name: 'Fraud', value: 13 },
    { name: 'Discrimination', value: 11 },
    { name: 'Ethics & Integrity', value: 8 },
    { name: 'Health & Safety', value: 6 },
    { name: 'Bribery', value: 1 },
    { name: 'Data Privacy', value: 1 },
  ];
  const option = {
    tooltip: { trigger: 'item', formatter: '{b}: {c} ({d}%)' },
    legend: { orient: 'vertical', left: 'left', top: 'middle', textStyle: { color: VIZ.axisLabel } },
    series: [
      {
        type: 'pie',
        radius: ['40%', '65%'],
        center: ['62%', '50%'],
        data: data.map((d, i) => ({ ...d, itemStyle: { color: VIZ.categorical[i % VIZ.categorical.length] } })),
        label: { show: false },
...
function CasesByOfficeChart() {
  const option = {
    ...
    yAxis: {
      type: 'category',
      data: ['Berlin', 'Singapore', 'New York', 'London'],
      axisLine: AXIS_DEFAULTS.axisLine,
      axisLabel: AXIS_DEFAULTS.axisLabel,
    },
    series: [
      {
        type: 'bar',
        data: [
          { value: 9, itemStyle: { color: VIZ.categorical[0], borderRadius: [0, 4, 4, 0] } },
          { value: 8, itemStyle: { color: VIZ.categorical[3], borderRadius: [0, 4, 4, 0] } },
          { value: 14, itemStyle: { color: VIZ.amber, borderRadius: [0, 4, 4, 0] } },
          { value: 23, itemStyle: { color: VIZ.alert, borderRadius: [0, 4, 4, 0] } },
        ],
...
function ReportBody() {
  return (
    <Stack gap={3}>
      ...
      <Typography variant="h2">1. Speak Up Programme</Typography>
      ...
      <Typography variant="textSm" color="text.secondary">
        Note: 15 cases are recorded with an Unknown Country and are excluded from the per-office columns above. Total lifetime case count across the programme
        is 67.
      </Typography>
      ...
      <Typography variant="body1">
        ...
        were recorded in 2022–2024 combined, but 5 cases were filed in 2025 and a further 3 in Q1 2026 alone. Harassment is concentrated heavily in London (17
        of 25 lifetime Harassment cases, 68%), which is structurally inconsistent with London representing only 27% of headcount.
      </Typography>
      ...
      <List dense disablePadding sx={{ pl: 2 }}>
        {[
          {
            label: 'London (GB):',
            text: '23 cases from ~180 employees, 12.8 per 100. The highest per-capita rate across all offices by a significant margin; 17 of those 23 cases are Harassment, indicating a localised conduct crisis rather than a general reporting culture effect.',
          },
          {
            label: 'New York (US):',
            text: '14 cases from ~420 employees, 3.3 per 100. The lowest per-capita rate. New York is the largest office (44% of headcount) but contributes only 20% of located cases. Underreporting is a credible concern.',
          },
          {
            label: 'Berlin (DE):',
            text: '9 cases from ~170 employees, 5.3 per 100. Moderate per-capita rate; case mix is relatively balanced across Discrimination, Fraud, and Health & Safety.',
          },
          {
            label: 'Singapore (SG):',
            text: '8 cases from ~180 employees, 4.4 per 100. Volume is comparable to Berlin but below London. 2 Health & Safety cases make this office the joint-highest per capita for that category.',
          },
        ].map(({ label, text }) => (
      ...
      <Typography variant="body1">
        The overall anonymous reporting rate is 38.8% (26 of 67 cases). Anonymity is channel-driven rather than category-driven: Open Reporting (100% anonymous,
        22 cases) and Vault Talk (100% anonymous, 4 cases) account for all 26 anonymous submissions, while the App and Manual Intake channels produce zero
        anonymous reports. At the category level, Harassment carries a 51.9% anonymous rate (14 of 27 cases), the highest of any high-volume category, and
        Discrimination sits at 45.5% (5 of 11). These rates signal that reporters in the two most sensitive conduct categories do not feel safe identifying
        themselves. Health &amp; Safety has a 0% anonymous rate across 6 cases, consistent with lower personal risk in those reports.
      </Typography>
      ...
      <Typography variant="body1">
        The average time to close a case is 42.8 days, and the average time from read to investigating is 4.6 days; both are operationally acceptable
        benchmarks. However, the open case aging profile is a material concern. Of 29 open cases as of 19 March 2026, 11 have been open for more than 180 days
        and a further 3 are aged between 91 and 180 days. That means 14 of 29 open cases (48.3%) have been open for more than 90 days. The Harassment category
        drives the most severe aging: 14 open Harassment cases include 4 aged beyond 180 days and 2 aged 91–180 days. Fraud also shows 4 cases beyond 180 days.
        The 11 cases open beyond 180 days represent a backlog that predates Q3 2025, suggesting that the programme's case throughput has not kept pace with the
        volume increase that began in Q2 2025.
      </Typography>
      ...
      <Typography variant="h2">2. Policy Programme</Typography>
      ...
      <Typography variant="body1">
        100% of the 50-policy portfolio is in Published status, which confirms procedural completeness at the point of initial approval. However, 20 of 50
        policies (40%) are overdue for review, with an average of 145.1 days overdue across the overdue set. The Harassment &amp; Workplace Conduct category is
        the most critically exposed: 5 of its 6 policies are overdue, including the Global Anti-Harassment Policy at 383 days, the Sexual Harassment Policy at
        329 days, and the UK Anti-Harassment Policy at 262 days. The Conflicts of Interest category has 3 of 4 policies overdue, and both Data Privacy &amp;
        Security (3 of 8) and HR &amp; Employment (3 of 7) carry significant overdue counts. The longest single overdue period belongs to the UK Data Privacy
        Policy (Post-Brexit) at 398 days, meaning Acme's post-Brexit UK data handling commitments have not been reviewed since February 2024. The only
        categories with zero overdue policies are Health &amp; Safety and Whistleblowing &amp; Speak Up.
      </Typography>
      ...
      <Typography variant="body1">
        ...
        Global Data Security Policy (211), and Gifts &amp; Entertainment Policy (203). Total portfolio access this quarter was 4,786 across 50 policies, an
        average of 95.7 accesses per policy. New York drives the highest absolute access volume (1,846), consistent with its headcount share. Berlin records the
        lowest total access at 686, which is disproportionately low even relative to its 170-person headcount (4.0 accesses per employee vs. 4.4 in Singapore,
        6.8 in London, and 4.4 in New York). Notably, no Harassment &amp; Workplace Conduct policy appears in the top-5 accessed list despite Harassment being
        the single largest case category.
      </Typography>
      ...
      <Typography variant="h2">3. Cross-Domain Risk Signals</Typography>
      ...
      {SIGNALS.map(({ title, severity, items }) => (
        <Stack key={title} gap={1}>
          ...
          <Typography variant="textSm" sx={{ fontWeight: 600, color: severity === 'HIGH' ? 'error.main' : 'warning.main' }}>
            Severity: {severity}
          </Typography>
          <List dense disablePadding sx={{ pl: 2 }}>
            {items.map((item) => (
      ...
      <Typography variant="h2">4. Recommended Actions</Typography>
